...
const CommentSection = ({ requestId, status, volunteerId }) => {
  const { authState } = useAuth();
  const toast = useToast();

  const isMyAssignment =
    authState.user?.role === "volunteer" && authState.user?.id === volunteerId;
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      toast({
        title: "Помилка",
        description:
          err.response?.data?.message || "Не вдалося додати коментар",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
      ...
      {authState.user?.role === "volunteer" &&
        ...
        !isMyAssignment && (
          <Text fontSize="sm" color="gray.600" mb={3}>
            Ви можете додати корисну інформацію або уточнення до замовлення,
            якщо маєте ідеї чи поради.
          </Text>
        )}
                    ...
                    <Box>
                      <Text fontWeight="bold">
                        ...
                        <Badge
                          ml={2}
                          colorScheme={
                            comment.user_role === "volunteer" ? "blue" : "teal"
                          }
                          fontSize="xs"
                        >
                          {comment.user_role === "volunteer"
