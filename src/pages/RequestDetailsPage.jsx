...
const RequestDetailsPage = () => {
  ...
  const isMyRequest =
    isRequester && request?.requester_id === authState.user?.id;
  const isMyAssignment =
    isVolunteer && request?.volunteer_id === authState.user?.id;
  const canComment =
    (isRequester && request?.requester_id === authState.user?.id) ||
    (isVolunteer &&
      (request?.volunteer_id === authState.user?.id ||
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      ...
      toast({
        title: "Помилка",
        description:
          err.response?.data?.message || "Не вдалося оновити статус замовлення",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
