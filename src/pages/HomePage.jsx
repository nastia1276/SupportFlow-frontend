...
const HomePage = () => {
    ...
    const fetchData = async () => {
      try {
        const [ordersRes, categoriesRes] = await Promise.all([
          axios.get("http://localhost:8000/api/v1/requests"),
  ...
  useEffect(() => {
    ...
    results = results.filter(
      (order) => order.status !== "Виконано" && order.status !== "Скасовано"
    );
    ...
    if (searchTerm) {
      ...
      results = results.filter(
        (order) =>
          ...
          (order.region &&
            order.region.toLowerCase().includes(lowerCaseSearchTerm))
      );
    }

    if (selectedCategory) {
      results = results.filter(
        (order) => order.category_id === parseInt(selectedCategory)
      );
    }
  ...
  const handleAcceptOrder = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:8000/api/v1/requests/${selectedOrder.id}/assign`,
        {},
        {
          headers: {
      ...
      toast({
        title: "Помилка",
        description:
          err?.response?.data?.message || "Не вдалося прийняти замовлення",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
  ...
  const greeting = authState.user
    ? `Вітаємо, ${authState.user.name} (${
        authState.user.role === "volunteer" ? "Волонтер" : "Отримувач"
      })`
    : "";
            ...
            <Stack spacing={5}>
              {filteredOrders.map((order) => (
                <Box
                  ...
                  border="1px solid"
                  borderColor="gray.200"
                  _hover={{ boxShadow: "md", borderColor: "black.300" }}
                  transition="all 0.2s"
                >
                  <Link
                  ...
                  >
                    ...
                    <Text mt={2}>
                      Категорія: {categoryMap[order.category_id] || "Невідома"}
                    </Text>
                    <Text>
                      Місто: {order.city || "—"}, Область: {order.region || "—"}
                    </Text>
                  ...
                  >
                    ...
                    <Flex gap={2} mt={{ base: 2, md: 0 }}>
                      {isLoggedIn &&
                        isVolunteer &&
                        order.status === "Очікує на виконавця" && (
                          <Button
                            size="sm"
                            colorScheme="green"
                  ...
                  {!isLoggedIn && (
                    <Text mt={2} fontSize="sm" color="gray.500">
                      Щоб взяти замовлення, будь ласка,{" "}
                      <Link
                        to="/login"
