...
const MyRequestsPage = () => {
  ...
  useEffect(() => {
    if (!authState.token) {
      toast({
        ...
        description:
          "Будь ласка, увійдіть в систему для перегляду ваших замовлень.",
        status: "warning",
        duration: 3000,
        isClosable: true,
      });
      ...
      const fetchUserProfile = async () => {
        try {
          ...
          toast({
            ...
            description:
              "Не вдалося отримати дані профілю. Будь ласка, увійдіть знову.",
            status: "error",
            duration: 3000,
            isClosable: true,
          });
    ...
    const fetchData = async () => {
      try {
        ...
        const errorMessage = err.response
          ? `Помилка: ${err.response.status} - ${
              err.response.data?.message || "Невідома помилка"
            }`
          : err.message ||
            "Не вдалося отримати дані замовлень. Перевірте з'єднання з сервером.";
  ...
  const userRole =
    authState.user?.role?.toLowerCase() === "volunteer"
      ? "volunteer"
      : "requester";
  const pageTitle =
    userRole === "volunteer" ? "Мої призначені замовлення" : "Мої замовлення";
        ...
        >
          <VStack spacing={5} align="stretch">
            ...
            <Text fontWeight="medium" color="gray.700" textAlign="center">
              ...
              {userRole === "volunteer" ? "Волонтер" : "Отримувач"})
            </Text>
              ...
              {requests.map((request) => (
                ...
                >
                  ...
                  <Flex mt={4} justify="space-between" align="center">
                    ...
                    {request.status === "Очікує на виконавця" &&
                      userRole === "requester" && (
                        <Button
                          size="sm"
                          colorScheme="red"
                          onClick={() =>
                            handleStatusChange(request.id, "Скасовано")
                          }
                        >
                    ...
                    {request.status === "В роботі" && (
                      <Button
                        ...
                        colorScheme="green"
                        onClick={() =>
                          handleStatusChange(request.id, "Виконано")
                        }
                      >
